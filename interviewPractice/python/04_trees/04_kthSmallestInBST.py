...
def kthSmallestInBST(t, k):
    r, smaller = inorderTraversal(t, k, 0)
    return r.value

    
def inorderTraversal(t, k, acum):
    if t == None:
        return None, acum
    
    r, smaller = inorderTraversal(t.left, k, acum)
    if r != None:
        return r, smaller 
    
    smaller += 1
    print(t.value, smaller)
    if smaller == k:
        return t, smaller
    
    return inorderTraversal(t.right, k, smaller)
