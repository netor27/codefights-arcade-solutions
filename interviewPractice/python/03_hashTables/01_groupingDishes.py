...
def groupingDishes(dishes):
    ...
    for d in dishes:
        ...
        for i in range(1, len(d)):
            if d[i] in ingredients:
                dishSet = ingredients[d[i]]
            else:
                dishSet = set()
            dishSet.add(dish)
            ingredients[d[i]] = dishSet
            
    items = sorted([x for x in ingredients.keys() if len(ingredients[x]) > 1])
    res = []
    
    for i in items:
        j = sorted(ingredients[i])
        res.append([i]+j)
    return res
