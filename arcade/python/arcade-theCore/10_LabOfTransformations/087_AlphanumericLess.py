...
def alphanumericLess(s1, s2):
    t1, t2 = getTokens(s1), getTokens(s2)
    t1len, t2len = len(t1), len(t2)
    n = min(t1len, t2len)    
    digitPos = None
    
    for i in range(n):
        c = compareTokens(t1[i], t2[i])
        if c == -1:
            return True
        elif c== 1:
            return False
        else:
            if t1[i].isdigit() and len(t1[i]) != len(t2[i]) and digitPos == None:
                digitPos = i
    
    if digitPos != None:
        return len(t1[digitPos]) > len(t2[digitPos])
